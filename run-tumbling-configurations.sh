...
run() {
  echo -n "$4 $3 $5"
  windowSize=$(($5 * 3))
  output=$(./cmake-build-debug/proofOfConcept --nodes $1 --thread-count $2 --merger $3 --marker $4 --run-length 20 --window-slide $5 --window-size $windowSize --shared-buffer-size 30000 2>&1)
  if [ $? -ne 0 ]; then
    echo $output
  fi
