...
template <typename KeyT, typename ValueT, typename HashT = std::hash<KeyT>, typename EqT = HashMapEqualTo<KeyT>, typename AggrT = DummyAggr<ValueT>>
...
public:
    HashTable (size_t size = MAP_SIZE) : _num_buckets(size), _mask(size-1) {
    ...
    HashTable (Bucket<KeyT, ValueT> *nodes, size_t size = MAP_SIZE) : _buckets(nodes), _num_buckets(size),
    _mask(size-1) {
        if (!(_num_buckets && !(_num_buckets & (_num_buckets - 1)))) {
            throw std::runtime_error ("error: the size of the hash table has to be a power of two\n");
        }
    ...
    void insert_or_modify (KeyT &key, ValueT &value, long timestamp) {
        ...
        /*for (int i = 0; i < _num_buckets; ++i) {
                printf("%d: state %d dirty %d, %d %d %d %d\n", i, _buckets[i].state, _buckets[i].dirty,
                       _buckets[i].key._0, _buckets[i].key._1, _buckets[i].key._2, _buckets[i].key._3);
        }*/
